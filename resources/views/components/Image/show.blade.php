...
<div class="d-flex align-items-center justify-content-center w-100">
    <a @if (asset($url) != asset('')) onclick="openImageModal(this)" data-bs-toggle="modal" href="#imageModal" role="button"@else style="cursor: default" @endif
        data-url="{{ asset($url) }}">
        <img class="{{ $classes }}" {{ $attributes->merge([]) }}
            @if (asset($url) != asset('')) src="{{ asset($url) }}" @else src="{{ asset('assets/images/defualt-user.png') }}" @endif
            alt="{{ ('No image found') }}" height="90" width="90" />
    </a>
</div>
